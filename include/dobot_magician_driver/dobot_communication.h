...
#include "dobot_magician_driver/serial_port.hpp"
...
protected:
  ...
  LibSerial::StopBits stop_bit_;
  LibSerial::CharacterSize character_size_;

  LibSerial::SerialPort *serial_port_;

  double serial_timeout_;
  int try_limit_;
  std::mutex communication_mt_;
  ...
  void packFromDouble(std::vector<double> &value_to_pack, std::vector<uint8_t> &packed_doubles);
  /**
